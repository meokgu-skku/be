...
class LikeRestaurantService(
    ...
    fun getLikeRestaurant(pageable: Pageable, email: String): GetLikeRestaurantsResponse {
        val user: User = userRepository.findByEmail(email) ?: throw NotFoundUserEmailException()
        // user 가 좋아요한 식당 아이디 리스트 조회
        val restaurantLikes: Page<RestaurantLike>? = restaurantLikeRepository.findByUserId(user.id, pageable)
        val restaurantIds: List<Long> = restaurantLikes?.map { it.restaurantId }?.toList() ?: emptyList()

        // 식당 아이디에 맞는 식당 리스트 반환
        val restaurants: List<RestaurantDto> = restaurantIds.map { restaurantRepository.findDtoById(it)?.toDto() ?: throw NotFoundRestaurantException() }
